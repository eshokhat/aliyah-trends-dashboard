...
def aliya_by_country(country):
    try:
        # Load data from the CSV file
        df = pd.read_csv(combined_table_path)
        
        # Filter data for the specified year
        year_data = df[df['erez_moza'] == country]
        ...
        plt.plot(years, olims, marker='o', linestyle='-', color='blue', label=f'Olims per Year in the {country}')
        
        # Adding labels and title
        plt.xlabel(f'{country}')
        plt.ylabel('Olims')
        plt.title(f'Olims Distribution for {country} by Year')
        plt.xticks(years)
        plt.legend()
