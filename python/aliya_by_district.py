...
def aliya_by_district(machoz):
    try:
        # Load data from the CSV file
        df = pd.read_csv(combined_table_path)
        
        # Filter data for the specified year
        year_data = df[df['machoz'] == machoz]
        ...
        if not years:
            print(f"No data found for {machoz}")
            return
        
        # Create a line plot using matplotlib
        plt.figure(figsize=(12, 6))
        plt.plot(years, olims, marker='o', linestyle='-', color='blue', label=f'Olims per Year in the {machoz}')
        
        # Adding labels and title
        plt.xlabel(f'{machoz}')
        plt.ylabel('Olims')
        plt.title(f'Olims Distribution for {machoz} by Year')
        plt.xticks(years)
        plt.legend()
